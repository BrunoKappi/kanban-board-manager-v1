...
export default function BoardCard({ Card, Index, ColumIndex, CardIndex }: Props) {
  ...
  return (
    <Draggable key={Card.CardId} draggableId={Card.CardId} index={Index}>
      {(DragProvided: any) => {
        return (
          <div className="group relative max-w-72 px-3 border line-clamp-2  text-card-foreground gap-2 flex flex-col items-start border-slate-200 bg-white  py-2   w-full rounded-md shadow-md hover:text-primary min-h-16 cursor-pointer dark:bg-overlay-dark dark:border-overlay-dark dark:text-primary-foreground flex-shrink" onClick={HandleSelectCard} ref={DragProvided.innerRef} style={DragProvided.draggableProps.style} {...DragProvided.draggableProps} {...DragProvided.dragHandleProps}>
            <div className="flex flex-row flex-wrap gap-1  gap-y-2 overflow-x-hidden max-w-full flex-shrink">
            ...
            <BoardCardOptions Card={Card} CardIndex={CardIndex} ColumIndex={ColumIndex} />
            {/* <CardModal Card={Card} Column={Column} ColumIndex={ColumIndex} CardIndex={CardIndex} /> */}
          </div>
        );
      }}
    </Draggable>
