...
export default function SearchBar({}: Props) {
  ...
  return (
    <div className="flex flex-row flex-grow max-w-[30%] gap-2 items-center">
      <div className="flex flex-row min-w-72  flex-grow  gap-2 items-center  px-3   bg-slate-400/10 dark:bg-slate-400/5 rounded-full">
        <Search />
        <MinimalInput maxLength={MAX_SEARCH} value={SearchFilter} className="bg-transparent border-none " placeholder="Search..." onChange={(e) => HandleSearch(e.target.value.trim())} />
      </div>
      <TagsFilter />
    </div>
  );
