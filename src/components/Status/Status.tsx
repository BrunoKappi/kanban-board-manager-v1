...
export default function Status({ Color = "neutral", Text = "Status", Column }: StatusProps) {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger>
        <Tooltip text="Clik to Edit">
          <div className={`flex flex-row gap-2 cursor-pointer items-center justify-center ${colors[Color.toLowerCase()].bg} px-3 py-0.5 rounded-full select-none`} onClick={() => setOpen(true)}>
            <div className={`size-3 ${colors[Color.toLowerCase()].circle} rounded-full`}></div>
            <span className={`${colors[Color.toLowerCase()].text} text-sm`}>{Text}</span>
          </div>
        </Tooltip>
      </PopoverTrigger>
      <PopoverContent className="w-56  py-2 bg-background flex flex-row flex-wrap dark:bg-background-dark dark:border-border-dark select-none gap-1">
        <MinimalInput maxLength={MAX_COLUMN_TITLE} value={ColumnTitle} onChange={(e) => setColumnTitle(e.target.value)} className={`p-0 px-3 py-0.5 h-auto text-sm rounded-full mb-5 mt-2 ${colors[ColorChoose.toLowerCase()].bg} ${colors[ColorChoose.toLowerCase()].text}`} autoFocus={false} />
